        ...
        <div class="select_box" ref="selectregionRef" :style="inputStyle" @click="showOptionsBut">
          <div class="select_input" v-bind:class="{ is_focused: isFocusOn }">
            ...
            <div class="select_but">
              <el-icon v-if="showOptions == false"><ArrowDown /></el-icon>
              <el-icon v-if="showOptions == true"><ArrowUp /></el-icon>
            </div>
          </div>
        </div>
      </el-form-item>
    ...
    if (res.code == 1) {
      ...
      for (let index = 0; index < res.posts.list.length; index++) {
        ...
        if (hasChooseList.value[element.id]) {
          hasChooseIds.value[element.id] = true;
        } else {
          hasChooseIds.value[element.id] = false;
        }
      }
...
function checkOneDo(e,r) {
  ...
  if (e) {
    hasChooseList.value[rg.id] = rg;
  } else {
    delete hasChooseList.value[rg.id];
  }

  //返回给上级组件所选地区id
  emit('saveChooseIds', Object.keys(hasChooseList.value));
}

//点击删除
function rmChooseOption(id) {
  if (hasChooseList.value[id]) {
    delete hasChooseList.value[id];
    hasChooseIds.value[id] = false;
